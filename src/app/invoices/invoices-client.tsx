    ...
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 self-start">
          ...
          <Link href="/invoices/new" className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors shadow-sm">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: "Te Paguara", value: invoices.filter((invoice) => invoice.status === "paid").length, color: "text-emerald-700" },
          { label: "Te Derguara", value: invoices.filter((invoice) => invoice.status === "sent").length, color: "text-blue-700" },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4">
          <p className="text-sm text-emerald-600 font-medium">Te Ardhura (Paguar)</p>
          <p className="text-2xl font-bold text-emerald-800 mt-1">EUR {totalRevenue.toFixed(2)}</p>
        </div>
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
          <p className="text-sm text-blue-600 font-medium">Ne Pritje</p>
          <p className="text-2xl font-bold text-blue-800 mt-1">EUR {pending.toFixed(2)}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Kerko fatura..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent" />
        </div>
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500">
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 text-left border-b border-slate-100">
                {["Nr. Fature", "Klienti", "Data Leshimit", "Shuma", "Statusi", "Veprime"].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {filtered.length === 0 ? (
                <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-400 text-sm">{search || filterStatus !== "all" ? "Nuk u gjeten fatura." : <>Nuk ka fatura. <Link href="/invoices/new" className="text-indigo-600 hover:underline">Krijo faturen e pare</Link></>}</td></tr>
              ) : (
                filtered.map((invoice) => (
                  <tr key={invoice.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4"><span className="font-mono text-sm font-semibold text-slate-800">{invoice.invoiceNumber}</span></td>
                    <td className="px-6 py-4"><Link href={`/clients/${invoice.client.id}`} className="text-sm font-medium text-slate-800 hover:text-indigo-600">{invoice.client.name}</Link></td>
                    <td className="px-6 py-4 text-sm text-slate-500">{format(new Date(invoice.issueDate), "d MMM yyyy", { locale: sq })}</td>
                    <td className="px-6 py-4 text-sm font-bold text-slate-900">EUR {invoice.total.toFixed(2)}</td>
                    <td className="px-6 py-4"><span className={`px-2.5 py-1 rounded-full text-xs font-medium ${statusColors[invoice.status]}`}>{statusLabel[invoice.status]}</span></td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-1">
                        ...
                        <button type="button" onClick={() => handleDelete(invoice)} disabled={deletingId === invoice.id} className="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="Fshij">
