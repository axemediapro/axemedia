...
function NewInvoiceForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch("/api/invoices", {
      ...
      body: JSON.stringify({ clientId, items, notes, taxRate }),
    });
    if (res.ok) {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl">
      <div className="flex items-center gap-3">
        <Link href="/invoices" className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Faturë e Re</h1>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">Gjenero faturë automatikisht</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          <h2 className="font-bold text-slate-900 mb-4 text-base">Detajet e Faturës</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <select value={clientId} onChange={(e) => setClientId(e.target.value)} required
                  className="flex-1 px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500"
                >
            ...
            <div>
              ...
              <input type="number" value={taxRate} onChange={(e) => setTaxRate(parseFloat(e.target.value) || 0)} min={0} max={100}
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500"
              />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-slate-900 text-base">Artikujt</h2>
            <button type="button" onClick={addItem} className="flex items-center gap-1.5 text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 font-semibold cursor-pointer">
          ...
          <div className="space-y-3">
            <div className="hidden sm:grid grid-cols-12 gap-2 text-xs font-semibold text-slate-500 uppercase px-2">
            ...
            {items.map((item, i) => (
              <div key={i}>
                {/* Mobile View */}
                <div className="sm:hidden bg-slate-50/80 border border-slate-200/80 rounded-2xl p-3.5 space-y-3">
                  <div>
                    <label className="block text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-1">Përshkrimi *</label>
                    <ServiceAutocomplete
                  ...
                  <div className="grid grid-cols-2 gap-2.5">
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-1">Sasia</label>
                      <input type="number" min={0.01} step="0.01" value={item.quantity}
                        onChange={(e) => updateItem(i, "quantity", parseFloat(e.target.value) || 0)} required
                        className="w-full px-3 py-2 border border-slate-200 rounded-xl text-sm bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 font-semibold"
                      />
                    </div>
                    <div>
                      <label className="block text-[11px] font-semibold text-slate-600 uppercase tracking-wider mb-1">Çmimi (€)</label>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-slate-200 text-xs">
                    <span className="font-medium text-slate-500">Totali: <strong className="text-slate-900 font-bold">€{(item.quantity * item.unitPrice).toFixed(2)}</strong></span>
                    {items.length > 1 && (
                      <button type="button" onClick={() => removeItem(i)} className="flex items-center gap-1 text-red-500 hover:text-red-700 font-semibold text-xs py-1 px-2 rounded-lg hover:bg-red-50 cursor-pointer">
                ...
                <div className="hidden sm:grid grid-cols-12 gap-2 items-center bg-slate-50/80 rounded-xl p-2 border border-slate-200/60">
                  ...
                  <div className="col-span-1 flex justify-end">
                    {items.length > 1 && (
                      <button type="button" onClick={() => removeItem(i)} className="p-1 text-slate-400 hover:text-red-500 transition-colors cursor-pointer">
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 flex justify-end">
            <div className="w-full sm:w-64 space-y-2 text-sm bg-slate-50 sm:bg-transparent p-3 sm:p-0 rounded-xl">
              ...
              <div className="flex justify-between font-bold text-slate-900 text-base border-t border-slate-200 pt-2 mt-2">
                <span>TOTALI:</span><span className="text-indigo-700 font-extrabold">€{total.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          ...
          <textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Shënime apo kushte pagese..."
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-2.5 sm:gap-3 pt-2">
          <Link href="/invoices" className="flex-1 sm:flex-none text-center px-4 py-2.5 border border-slate-200 text-slate-600 rounded-xl text-xs sm:text-sm font-semibold hover:bg-slate-50 transition-colors">Anulo</Link>
          <button type="submit" disabled={loading} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl text-xs sm:text-sm font-semibold hover:bg-indigo-700 transition-colors disabled:opacity-60 shadow-sm shadow-indigo-600/20 cursor-pointer">
            <Save className="w-4 h-4" />{loading ? "Duke krijuar..." : "Krijo Faturën"}
          </button>
        </div>
      </form>
    </div>
