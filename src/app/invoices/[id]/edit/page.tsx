  ...
  useEffect(() => {
    Promise.all([
      ...
      if (inv && !inv.error) {
        ...
        setNotes(inv.notes || "");
        // Reverse-engineer taxRate from subtotal/tax
        const rate = inv.subtotal > 0 ? Math.round((inv.tax / inv.subtotal) * 100) : 18;
        setTaxRate(rate);
        setItems(
          inv.items.length > 0
            ? inv.items.map((it: { description: string; quantity: number; unitPrice: number }) => ({
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        <Link href={`/invoices/${id}`} className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Ndrysho Faturën</h1>
          <p className="text-slate-500 text-xs sm:text-sm mt-0.5">Edito të gjitha të dhënat e faturës</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4">
            <div>
              <label className="block text-xs sm:text-sm font-semibold text-slate-700 mb-1.5">Nr. Faturës *</label>
              <input
                type="text"
                value={invoiceNumber}
                onChange={(e) => setInvoiceNumber(e.target.value)}
                required
                className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 font-mono font-semibold"
              />
            </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <button type="button" onClick={addItem}
              className="flex items-center gap-1.5 text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 font-semibold cursor-pointer"
            >
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 sm:p-6">
          ...
          <textarea rows={3} value={notes} onChange={(e) => setNotes(e.target.value)}
            placeholder="Shënime apo kushte pagese..."
