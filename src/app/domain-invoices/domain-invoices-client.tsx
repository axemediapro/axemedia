...
export default function DomainInvoicesClient({ initialReminders, initialInvoices, initialSettings }: { initialReminders: Reminder[]; initialInvoices: SavedInvoice[]; initialSettings: Settings }) {
  ...
  return (
    <div className="domain-invoice-shell min-h-full bg-[#eef4f7] p-5 lg:p-8">
      <div className="mx-auto max-w-[1500px] space-y-6">
        <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-black tracking-tight text-slate-950">Domain & Hosting Invoice</h1>
            <p className="mt-1 text-sm text-slate-500">Create a dedicated invoice for domains, hosting and web infrastructure.</p>
          </div>
          <div className="flex flex-wrap gap-2"><button onClick={() => void saveInvoice()} disabled={savingInvoice} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-bold text-white shadow-lg transition hover:bg-emerald-700 disabled:opacity-60"><Save className="h-4 w-4" /> {savingInvoice ? "Saving..." : savedInvoiceId ? "Update invoice" : "Save invoice"}</button>{savedInvoiceId && <button onClick={() => void duplicateInvoice()} disabled={savingInvoice} className="flex items-center justify-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-sm transition hover:border-cyan-400 hover:text-cyan-700 disabled:opacity-60"><Copy className="h-4 w-4" /> Duplicate invoice</button>}<button onClick={printInvoice} className="flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-5 py-3 text-sm font-bold text-white shadow-lg shadow-slate-900/15 transition hover:bg-cyan-800"><FileDown className="h-4 w-4" /> Save as PDF</button></div>
        </header>
        ...
        <div className="grid gap-6 xl:grid-cols-[390px_minmax(0,1fr)]">
          <aside className="space-y-5 print:hidden">
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="space-y-3">
                <div className="flex items-end gap-2"><label className="block min-w-0 flex-1 text-xs font-bold uppercase tracking-wide text-slate-500">Saved domains</label><button type="button" onClick={() => setShowNewDomain((current) => !current)} className="flex items-center gap-1 rounded-lg bg-cyan-50 px-2.5 py-1.5 text-xs font-bold text-cyan-700 hover:bg-cyan-100"><Plus className="h-3.5 w-3.5" /> Add new</button></div>
                ...
                <div className="grid grid-cols-2 gap-3"><label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">Service type</span><select value={form.serviceType ?? "hosting"} onChange={(event) => { const serviceType = event.target.value; setForm((current) => ({ ...current, serviceType, service: serviceType === "domain" ? "Domain renewal" : serviceType === "all" ? "Domain & hosting renewal" : "Managed hosting" })); }} className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm"><option value="domain">Domain</option><option value="hosting">Hosting</option><option value="all">All (Domain & Hosting)</option></select></label><Field label="Domain / service name" value={form.domainName} onChange={(value) => updateForm("domainName", value)} placeholder="example.com" /></div>
                ...
                <div className="grid grid-cols-3 gap-3"><Field label="Period" value={form.period} onChange={(value) => updateForm("period", value)} /><Field label="Quantity" value={form.quantity} onChange={(value) => updateForm("quantity", value)} type="number" /><Field label="Unit price" value={form.amount} onChange={(value) => updateForm("amount", value)} type="number" /></div>
            ...
            <button type="button" onClick={() => setShowHistory(true)} className="flex w-full items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:border-cyan-300 hover:shadow-md">
              <span><span className="block font-bold text-slate-900">Billing history</span><span className="mt-1 block text-sm text-slate-500">Open and edit all saved invoices</span></span>
          ...
          <main className="min-w-0">
            <div className="domain-invoice-document overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-xl print:rounded-none print:border-0 print:shadow-none">
              <article className="relative min-h-[920px] overflow-hidden p-8 sm:p-12 print:min-h-0" style={{ ...invoiceStyle, ...(settings.domainInvoiceBackgroundUrl ? { backgroundImage: `linear-gradient(rgba(255,255,255,.93),rgba(255,255,255,.93)), url(${settings.domainInvoiceBackgroundUrl})`, backgroundSize: "cover", backgroundPosition: "center" } : {}) }}>
                ...
                <div className="relative">
                  <div className="flex flex-col justify-between gap-8 sm:flex-row"><div><EditableText value={form.providerName || "Service provider"} onChange={(value) => updateForm("providerName", value)} className="text-xs font-black uppercase tracking-[0.24em]" style={{ color: mainColor }} /><h2 className="mt-3 text-4xl font-black leading-[0.75] tracking-tight text-slate-950"><EditableText value={form.serviceType === "domain" ? "DOMAIN" : form.serviceType === "all" ? "DOMAIN & HOSTING" : "HOSTING"} onChange={(value) => updateForm("serviceType", value.toLowerCase().includes("domain") && value.toLowerCase().includes("hosting") ? "all" : value.toLowerCase() === "domain" ? "domain" : "hosting")} /><br /><EditableText value="RENEWAL" onChange={() => undefined} className="mt-0" style={{ color: mainColor }} /></h2></div>{settings.domainInvoiceLogoUrl ? <img src={settings.domainInvoiceLogoUrl} alt="Logo" className="h-20 max-w-[220px] object-contain object-left sm:object-right" /> : <div className="flex h-20 w-40 items-center justify-center border border-dashed border-slate-300 text-xs font-bold uppercase tracking-widest text-slate-300">Logo</div>}</div>
                  <div className="mt-12 grid gap-3 sm:grid-cols-3"><Info label="Invoice number" value={form.invoiceNumber} /><Info label="Issue date" value={format(new Date(form.issueDate), "dd MMM yyyy", { locale: enUS })} /><Info label="Status" value={(form.status ?? "unpaid") === "paid" ? "PAID" : "UNPAID"} accentColor={(form.status ?? "unpaid") === "paid" ? "#16A34A" : "#DC2626"} /></div>
                  <div className="mt-12 grid gap-8 border-y border-slate-200 py-7 sm:grid-cols-2"><div><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Billed to</p><EditableText value={form.customer || "Customer name"} onChange={(value) => updateForm("customer", value)} className="mt-2 text-lg font-black text-slate-950" /><EditableText value={form.customerEmail || "email@customer.com"} onChange={(value) => updateForm("customerEmail", value)} className="mt-1 text-sm text-slate-500" /></div><div className="sm:text-right"><p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">Service details</p><EditableText value={form.domainName || form.service || "Domain or hosting service"} onChange={(value) => updateForm("domainName", value)} className="mt-2 ml-auto text-lg font-black text-slate-950" /><div className="mt-1 flex justify-end text-sm text-slate-500"><span>Period:&nbsp;</span><EditableText value={form.period || "12 months"} onChange={(value) => updateForm("period", value)} />{selectedReminder?.nextDueDate && <span>&nbsp;· renewal {format(new Date(selectedReminder.nextDueDate), "dd MMM yyyy", { locale: enUS })}</span>}</div></div></div>
                  <div className="mt-10"><div className="grid grid-cols-[1fr_110px_150px] gap-7 border-b border-slate-950 pb-3 text-[11px] font-black uppercase tracking-[0.16em] text-slate-500"><span>Description</span><span className="text-center">Quantity</span><span className="text-right">Amount</span></div><div className="grid grid-cols-[1fr_110px_150px] items-center gap-7 border-b border-slate-200 py-6"><div><p className="font-bold text-slate-950">{form.service || (form.serviceType === "domain" ? "Domain renewal" : form.serviceType === "all" ? "Domain & hosting renewal" : "Managed hosting")}</p><p className="mt-1 text-sm text-slate-500">{form.domainName || "Domain or service name"} · {form.period || "12 months"}</p></div><p className="text-center font-bold text-slate-950">{quantity}</p><p className="text-right font-black text-slate-950">{subtotal.toFixed(2)} {form.currency}</p></div></div>
                  <div className="ml-auto mt-8 max-w-sm space-y-3"><div className="flex justify-between text-sm text-slate-500"><span>Subtotal</span><span>{subtotal.toFixed(2)} {form.currency}</span></div><div className="flex justify-between text-sm text-slate-500"><span>Tax ({taxRate}%)</span><span>{taxAmount.toFixed(2)} {form.currency}</span></div><div className="flex justify-between border-t border-slate-950 pt-4 text-xl font-black text-slate-950"><span>Total due</span><span style={{ color: mainColor }}>{total.toFixed(2)} {form.currency}</span></div></div>
                  <div className="mt-20 grid gap-8 border-t border-slate-200 pt-6 text-xs text-slate-500 sm:grid-cols-2"><div><EditableText value="Business Information" onChange={() => undefined} className="font-black uppercase tracking-widest text-slate-950" /><EditableText value={form.businessName || "Business name"} onChange={(value) => updateForm("businessName", value)} className="mt-2" /><EditableText value={form.businessAddress || "Business address"} onChange={(value) => updateForm("businessAddress", value)} /><EditableText value={form.businessEmail || "business@email.com"} onChange={(value) => updateForm("businessEmail", value)} /><EditableText value={form.businessPhone || "Business phone"} onChange={(value) => updateForm("businessPhone", value)} /></div><div className="sm:text-right"><EditableText value="Note" onChange={() => undefined} className="ml-auto font-black uppercase tracking-widest text-slate-950" /><EditableText value={form.notes || "-"} onChange={(value) => updateForm("notes", value)} className="mt-2 ml-auto leading-5" /></div></div>
                </div>
              </article>
            </div>
          </main>
