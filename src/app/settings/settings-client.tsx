  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl space-y-6">
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">Këto të dhëna shfaqen automatikisht në faturat dhe ofertat e gjeneruara</p>
      </div>

      <form onSubmit={handleSave} className="space-y-5">
        {error && <div className="bg-red-50 border border-red-200 text-red-700 rounded-lg px-4 py-3 text-sm">{error}</div>}
        {saved && <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg px-4 py-3 text-sm flex items-center gap-2"><CheckCircle className="w-4 h-4" /> Cilësimet u ruajtën me sukses!</div>}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => logoRef.current?.click()} disabled={uploading} className="flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-lg text-sm font-medium hover:bg-indigo-100 transition-colors disabled:opacity-60"><Upload className="w-4 h-4" />{uploading ? "Duke ngarkuar..." : "Ngarko Logo"}</button>
              {form.logoUrl && <button type="button" onClick={() => setForm({ ...form, logoUrl: "" })} className="flex items-center gap-1.5 text-sm text-red-500 hover:text-red-600"><X className="w-3.5 h-3.5" /> Fshij logon</button>}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <div className="space-y-2">
              ...
              <button type="button" onClick={() => stampRef.current?.click()} disabled={uploadingStamp} className="w-full px-3 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200 disabled:opacity-60">{uploadingStamp ? "Duke ngarkuar..." : "Ngarko Stampë"}</button>
            </div>
            <div className="space-y-3 sm:col-span-2">
              ...
              <button type="button" onClick={() => signatureRef.current?.click()} disabled={uploadingSignature} className="w-full px-3 py-2 bg-slate-100 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-200 disabled:opacity-60">{uploadingSignature ? "Duke ngarkuar..." : "Ngarko Nënshkrim"}</button>
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              ...
              <div className="flex items-center gap-3">
                <input type="color" value={form.primaryColor} onChange={(e) => setForm({ ...form, primaryColor: e.target.value })} className="w-12 h-10 rounded-lg cursor-pointer border border-slate-200 p-0.5" />
                <span className="text-sm font-mono text-slate-600">{form.primaryColor}</span>
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Madhësia e Logos: <span className="font-semibold" style={{ color: form.primaryColor }}>{form.logoSize} mm</span></label>
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Fonti i PDF-ve</label>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {FONT_OPTIONS.map((font) => (
                <button key={font.value} type="button" onClick={() => setForm({ ...form, fontFamily: font.value })} className={`text-left rounded-xl border p-3 transition-colors ${form.fontFamily === font.value ? "border-indigo-300 bg-indigo-50" : "border-slate-200 bg-white hover:bg-slate-50"}`}>
                  <div className="font-medium text-slate-900" style={{ fontFamily: font.css }}>{font.label}</div>
                  <div className="text-xs text-slate-500 mt-1">{font.subtitle}</div>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="flex justify-end gap-2">
          <button type="button" onClick={() => setForm(initialSettings)} className="px-4 py-2 border border-slate-200 text-slate-600 rounded-lg text-sm hover:bg-slate-50">Rivendos</button>
          <button type="submit" disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 transition-colors disabled:opacity-60"><Save className="w-4 h-4" />{saving ? "Duke ruajtur..." : "Ruaj Ndryshimet"}</button>
        </div>
      </form>
    </div>
  );
