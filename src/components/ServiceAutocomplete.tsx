            ...
            filtered.map((s, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full px-3 py-2.5 text-left text-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-1 sm:gap-2 transition-colors border-b border-slate-100/60 last:border-none ${
                    idx === activeIdx ? "bg-indigo-50" : "hover:bg-slate-50 active:bg-indigo-50"
                  }`}
                >
                  <div className="flex flex-col min-w-0">
                    <span className="font-semibold text-slate-800 text-xs sm:text-sm">{s.name}</span>
                    {s.description && (
                  ...
                  <div className="flex-shrink-0 text-right self-end sm:self-center">
                    {isTiered ? (
                      <div className="flex items-center gap-1 text-[10px] sm:text-[11px] font-semibold text-indigo-700 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100">
                        <Clock className="w-3 h-3 text-indigo-500 flex-shrink-0" />
                        <span>1h: €{s.price1h ?? 70} · 2-5h: €{s.price2to5h ?? 60}</span>
                      </div>
                    ) : (
